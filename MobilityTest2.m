...
initialCon = [];
for i = 1:numel(X)
    spheres.addSwimmer(prolateSpheroid(0.15,1,0.05),[X(i), Y(i), 0],[1;0;0;0;1;0]);
...
swimmers.genTree('nodeCapacity',2000);
swimmers.genKIFMM([1e-6, 1],'parThreads',4,'GPU',1,'GMRES',1);
...
x = gmres(afunc, [vel-shear;zeros(6*swimmers.swimmerNo(),1)],100,1e-4,2);

swimmers.genKIFMM([1e-3, 1],'parThreads',4,'GPU',1,'GMRES',1);

Ax0 = afunc(x);
scale = ([vel-shear;zeros(6*swimmers.swimmerNo(),1)].'*Ax0)/norm(Ax0)^2;

x = gmres(afunc, [vel-shear;zeros(6*swimmers.swimmerNo(),1)],100,1e-4,2, [], [], scale * x);
