...
initialCon = [];
for i = 1:numel(X)
    coarse = prolateSpheroid(0.15,1,0.36);
    fine = [prolateSpheroid(0.15,1,0.18);coarse];
    NN = nearestMatrix(coarse,fine,1,0);
    vel = zeros(size(coarse));
    spheres.addSwimmer(coarse,fine,NN,[X(i), Y(i), 0],[1;0;0;0;1;0],vel,...
                       [0;0;0],[0;0;0]);
...
% 
% writematrix(dataPre.',"preconNearJoint.csv")
...
for i = 1:numel(X)
    coarse = prolateSpheroid(0.15,1,0.36);
    fine = prolateSpheroid(0.15,1,0.18);
    NN = nearestMatrix(coarse,fine,1,0);
    vel = zeros(size(coarse));
    spheres.addSwimmer(coarse,fine,NN,[X(i), Y(i), 0],[1;0;0;0;1;0],vel,...
                       [0;0;0],[0;0;0]);
...
writematrix(file,"preconNearDisjoint.csv")
...
swimmers.genTree('nodeCapacity',500,'nearest',1);
...
pfunc = @(x) mobilityPrecon(x, swimmers,B,BT,A1,A2,1e-6,100);
