...
for i = 1:numel(X)
    ...
    tic
    tree = OcTree(points,points,'nodeCapacity',500,'parThreads',12);
    fmm = KIFMM(tree,[Y(i),1],'GPU',1,'parThreads',4,'GMRES',1);
    ...
    points = gpuArray(reshape(points.',[],1));
    vel = gpuArray(vel);
    
    tic
    [pot2,~,~,iter2] = gmres(@(x) blockcomputation(points,points,x,0.2,[Y(i),1]),vel,[],[],500);
    timeinverse2 = toc;

    pot2 = gather(reshape(pot2,3,[]).');
    ...
    csvwrite('DirectMethodComparisonGPU.csv',storage)
end